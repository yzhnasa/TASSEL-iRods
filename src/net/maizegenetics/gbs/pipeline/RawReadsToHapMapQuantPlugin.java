...
//            short[][][][] alleleCounts = setUpAlleleCounts(outMSA);
            ...
            try {
                while ((temp = br.readLine()) != null) {
                    ...
                    if (rr != null) {
                        ...
                        recordVariantsFromTag(alleleList, theTOPM, outMSA, tagIndex, taxaNameIndices.whichIdNumber(rr.getTaxonName()));
    ...
    private MutableNucleotideAlignment[] setUpMutableNucleotideAlignments(ParseBarcodeRead thePBR, TagsOnPhysicalMap theTOPM) {
        ...
        taxaNameIndices = outMSA[0].getIdGroup();
    ...
    private short[][][][] setUpAlleleCounts(MutableNucleotideAlignment[] outMSA) {
    ...
    private void recordVariantsFromTag(TagsOnPhysicalMap theTOPM, MutableNucleotideAlignment[] outMSA, int tagIndex, int taxonIndex) {
        ...
        for (int variant = 0; variant < theTOPM.getMaxNumVariants(); variant++) {
            ...
            if (prevBase == Alignment.UNKNOWN_DIPLOID_ALLELE) {
                ...
                outMSA[chrIndex].setBase(taxonIndex, currSite, DiscoverySNPCallerPlugin.resolveSNPByteFromCallPair(prevBase, currBase));
            }
        }
    }

    private void recordVariantsFromTag(short[][][][] alleleCounts, TagsOnPhysicalMap theTOPM, MutableNucleotideAlignment[] outMSA, int tagIndex, int taxonIndex) {
        int chromosome = theTOPM.getChromosome(tagIndex);
        if (chromosome == Integer.MIN_VALUE) {
            return;
        }
    ...
    private void recordVariantsFromTag(ArrayList<String>[][] alleleList, TagsOnPhysicalMap theTOPM, MutableNucleotideAlignment[] outMSA, int tagIndex, int taxonIndex) {
        int chromosome = theTOPM.getChromosome(tagIndex);
        if (chromosome == Integer.MIN_VALUE) {
            return;
        }
    ...
    private void callSNPsQuantitatively(short[][][][] alleleCounts, MutableNucleotideAlignment[] outMSA) {
    ...
    private void callSNPsQuantitatively(ArrayList<String>[][] alleleList, MutableNucleotideAlignment[] outMSA) {
        myLogger.info("\nPerforming quantitative SNP calling...");
        int nSites = 0;
    ...
    private byte resolveQuantGeno(short[] alleleCounts) {
        ...
        if (totCount < maxCountAtGeno) {
            if (a2Count < likelihoodRatioThreshAlleleCnt[totCount]) {
                ...
                return AlignmentUtils.getDiploidValue(a1, a2);
            }
        } else {
