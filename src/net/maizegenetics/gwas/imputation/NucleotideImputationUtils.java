...
public class NucleotideImputationUtils {
	...
	public static void callParentAlleles(PopulationData popdata, int minAlleleCount, int windowSize, int numberToTry, double cutHeightSnps, double minR) {
		...
		GenotypeTable aAlignment = FilterAlignment.getInstance(popdata.original, parentAGroup);
		GenotypeTable cAlignment = FilterAlignment.getInstance(popdata.original, parentCGroup);
		...
		//SBitAlignment sbitPopAlignment = SBitAlignment.getInstance(popdata.original);
                GenotypeTable sbitPopAlignment = ConvertSBitTBitPlugin.convertAlignment(popdata.original, ConvertSBitTBitPlugin.CONVERT_TYPE.sbit, null);
...
//			mna.clean();
			...
			GenotypeTable windowAlignment = FilterAlignment.getInstance(popdata.original, snpIndex);
...
//				tdp.performFunction(new DataSet(new Datum("Snp Tree", myTree, "Snp Tree"), null));
		...
		GenotypeTable target = FilterAlignment.getInstance(popdata.original, snpIndex);
	...
	public static void callParentAllelesUsingClusters(PopulationData popdata, double maxMissing, double minMaf, int windowSize) {
		...
		GenotypeTable a = AlignmentUtils.removeSitesBasedOnFreqIgnoreMissing(popdata.original, minMaf, 1.0, minCount);
		...
		while (maxdist > 10) {
			...
			int mindist2, mindist3;
			mindist2 = Math.min(HaplotypeClusterer.clusterDistanceMaxPairDiff(clusters.get(0), clusters.get(2)), 
					HaplotypeClusterer.clusterDistanceMaxPairDiff(clusters.get(1), clusters.get(2)));
			mindist3 = Math.min(HaplotypeClusterer.clusterDistanceMaxPairDiff(clusters.get(0), clusters.get(3)),
		...
		HaplotypeCluster.ReturnHaplotype = HaplotypeCluster.TYPE.majority;
		System.out.printf("%s\n%s\n%s\n%s\n", clusters.get(0), clusters.get(1), clusters.get(2), clusters.get(3));
		...
		while (siteNumber[0] == windowSize && sites[windowSize - 1] < nFilteredSites - 1) {
			...
			System.out.printf("At site = %d haplotypes are\n", sites[0]);
			System.out.println(seedClusters.get(0));
			System.out.println(seedClusters.get(1));
...
//	 	exportParentHaplotypes("/Volumes/Macintosh HD 2/temp/z1chr1parents.hmp.txt", popdata, "1");
	...
	public static void checksubpops(PopulationData popdata, int halfWindowSize) {
		...
		for (int i = 0; i < 3; i++) {
			GenotypeTable a = FilterAlignment.getInstance(popdata.original, new SimpleIdGroup(subids.get(i)));
	...
	public static ArrayList<HaplotypeCluster> extendClusters(GenotypeTable a, ArrayList<HaplotypeCluster> clusters, int[] sites, int[] windowSize, boolean forward) {
		...
		int ntaxa = a.numberOfTaxa();
		GenotypeTable b = FilterAlignment.getInstance(a, selectedSites);
		for (int t = 0; t < ntaxa; t++) {
	...
	public static void callParentAllelesByWindowForBackcrosses(PopulationData popdata, double maxMissing, double minMaf, int windowSize, double minR) {
		...
		GenotypeTable target2 = FilterAlignment.getInstance(target, targetids);
...
//			myLogger.info(strb.toString());
	...
	public static void examineTaxaClusters(GenotypeTable a, BitSet polybits) {
		...
		while (sitecount < nsnps) {
			...
			if (sitecount < nsnps) {
				GenotypeTable subAlignment = BitAlignment.getInstance(FilterAlignment.getInstance(a, snpndx), true);
				...
				while (true) {
					...
					if (((double) grpSize[order[0]]) / ((double) grpSize[order[1]]) < 2.0) {
						...
						GenotypeTable alignA = FilterAlignment.getInstance(subAlignment, new SimpleIdGroup(taxaA));
						GenotypeTable alignB = FilterAlignment.getInstance(subAlignment, new SimpleIdGroup(taxaB));
