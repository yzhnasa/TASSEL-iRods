...
public class PrincipalComponents {
    ...
    public PrincipalComponents(DoubleMatrix2D genotypes, int type) {
        ...
        if (type == TYPE_CENTER) {
            centerGenotypesByMarker();
        } else if (type == TYPE_CENTER_AND_SCALE) {
            centerAndScaleGenotypes();
        }
        ...
        if (nrows < ncols) {
            ...
            svd = new SingularValueDecomposition(genotypes);
        }
    ...
    public DoubleMatrix2D getPrincipalComponents() {
        if (transposed) {
            return svd.getV().zMult(svd.getS(), null).viewDice().copy();
        } else {
            return svd.getU().zMult(svd.getS(), null).viewDice().copy();
        }
    }
