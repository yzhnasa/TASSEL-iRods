...
//			testMarkers = false;
	...
	public List<Datum> solve(){
    	...
    	for (int ph = 0; ph < numberOfPhenotypes; ph++) {
    		...
    		int nonMissingObs = 0;
    		for (boolean m : missing) if (!m) nonMissingObs++;
    		...
    		for (int i = 0; i < totalObs; i++) {
    			if (!missing[i]) Z.set(count++, kin.whichIdNumber(theTaxa[i]), 1);
    		}
			...
			if (parentPlugin.isWriteOutputToFile()) {
				if (!writeObjectToFile(mainOutputWriter, tableRow)) return null;
			} else {
			...
			if (useP3D) Vminus = new SymmetricMatrixInverterDM(calculateV(ZKZ, genvar, resvar));
			...
			if (testMarkers) {
				for (int m = 0; m < numberOfMarkers; m++) {
	    			...
	    			for (boolean miss:additionalMissing) if(!miss) numberOfRowsKept++;
	    			int[] keepIndex = new int[numberOfRowsKept];
	    			count = 0;
	    			for (int i = 0; i < nonMissingObs; i++) if(!additionalMissing[i]) keepIndex[count++] = i;  
	    			
	    			DoubleMatrix ymarker = y.getSelection(keepIndex, null);
	    			count = 0;
	    			for (int i = 0; i < totalObs; i++) {
	    				if (!finalMissing[i]) ymarker.set(count++, 0,phenotypeData[i]);
	    			}
	    			...
	    			if (markerIsDiscrete) {
		    			...
		    			for (int i = 0; i < n; i++) {
		    				if (!finalMissing[i]) finalMarkerData[count++] = markerData[i]; 
		    			}
		    			...
		    			int n = markerData.length;
		    			for (int i = 0; i < n; i++) {
		    				if (!finalMissing[i]) finalMarkerValues[count++] = (Double) markerData[i]; 
		    			}
					...
					if (parentPlugin.isFilterOutput() && result.p > parentPlugin.getMaxp()) recordTheseResults = false;
					...
					if (recordTheseResults) {
						...
						if (!markerIsDiscrete) {
							...
							//record the results
							if (parentPlugin.isWriteOutputToFile()) {
								if (!writeObjectToFile(alleleOutputWriter, tableRow)) return null;
							} else {
							...
							for (int a = 0; a < nAlleles; a++) {
								Double estimate;
								if (a < nAlleles - 1) estimate = result.beta.get(result.beta.numberOfRows() - nAlleles + 1 + a, 0);
								else estimate = 0.0;
								if (myGeneticMap != null) {
									tableRow = new Object[] {theAdapter.getPhenotypeName(ph),
											markername,
    	...
    	if (parentPlugin.isWriteOutputToFile()) {
    		if (useCompression) writeCompressionResultsToFile();
    		return null;
    	}
	...
	private String getTabbedStringFromArray(Object[] array) {
		...
		sb.append(array[0]);
		int n = array.length;
		for (int i = 1; i < n; i++) sb.append("\t").append(array[i]);
		return sb.toString();
	}
	...
	public List<Datum> formatResults() {
		...
		options.append("Compression = ").append(useCompression);
		if (useCompression) options.append(", compression level = ").append(compression).append("\n");
		if (useP3D) options.append("P3D = ").append(useP3D).append(". Variance components were estimated only for the model without any markers.\n");
		else options.append("P3D = ").append(useP3D).append(". Variance components were estimated for each marker.\n");
		
		StringBuilder model = new StringBuilder();
		model.append("Model: trait = mean + ");
		int nFactors = theAdapter.getNumberOfFactors();
		for (int f = 0; f < nFactors; f++) model.append(theAdapter.getFactorName(f)).append(" + ");
		int nCovar = theAdapter.getNumberOfCovariates();
		for (int c= 0; c < nCovar; c++) model.append(theAdapter.getCovariateName(c)).append(" + ");
		model.append("marker\n");
	...
	public void computeZKZ(DoubleMatrix data, DoubleMatrix X, DoubleMatrix Z, DistanceMatrix kin, String traitname) {
		...
		if (!useCompression) {
			...
			while (count < n && !taxaReplicated) if (Z.columnSum(count++) > 1.5) taxaReplicated = true;
...
//			CompressedDoubleMatrix cm = new CompressedDoubleMatrix(kin, new NeighborJoiningTree(distance));
	...
	private void calculateRsquare(DoubleMatrix X, DoubleMatrix y, DoubleMatrix invV, CompressedMLMResult result, int markerdf) {
		...
		for (int i = 0; i < dimXreduced; i++) colsToKeep[i] = i;
	...
	public DistanceMatrix calculateDistanceFromKin(DistanceMatrix kin) {
		...
		for (int i = 0; i < n; i++) max = Math.max(max, kin.getDistance(i, i));
			
		double constant;
		if (max > 2) constant = max;
		else if (max > 1 ) constant = 2;
		else constant = 1;
    ...
    public IdGroup updateMissingWithKinship(boolean[] missing, Identifier[] phenotypeTaxa) {
		...
		for (int i = 0; i < n; i++) {
			...
			if (!missing[i]) {
				if (col == -1) missing[i] = true;
				else kinSet.add(phenotypeTaxa[i]);
			}
		}
