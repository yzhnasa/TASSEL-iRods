...
public class MarkerPhenotypeAdapterUtils {
    ...
    public static void updateMissing(boolean[] missing1, boolean[] missing2) {
    	...
    	if (missing2.length != n) throw new IllegalArgumentException("missing1 and missing2 must be the same length");
    	for (int i = 0; i < n; i++) missing1[i] = missing1[i] || missing2[i];
    }
    ...
    public static void updateMissing(boolean[] missing, double[] data) {
    	...
    	if (data.length != n) throw new IllegalArgumentException("missing and data must be the same length");
    	for (int i = 0; i < n; i++) missing[i] = missing[i] || Double.isNaN(data[i]);
    }
    ...
    public static void updateMissing(boolean[] missing, String[] labels) {
    	int n = missing.length;
    	if (labels.length != n) throw new IllegalArgumentException("missing and labels must be the same length");
    	for (int i = 0; i < n; i++) missing[i] = missing[i] || isMissing(labels[i]);
    }
	...
	public static boolean[] whichAreMissing(double[] doubleArray) {
		...
		for (int i = 0; i < n; i++) missing[i] = Double.isNaN(doubleArray[i]);
		return missing;
	}
	...
	public static boolean[] whichAreMissing(String[] stringArray) {
		...
		boolean[] missing = new boolean[n];
		for (int i = 0; i < n; i++) missing[i] = (isMissing(stringArray[i])) ;
		return missing;
	}
	...
	public static boolean[] whichAreMissing(Object[] values) {
		...
		if (values instanceof String[]) {
			...
			for (int i = 0; i < n; i++) missing[i] = isMissing(strvalues[i]);
		} else if (values instanceof Double[]) {
			Double[] dblvalues = (Double[]) values;
			for (int i = 0; i < n; i++) missing[i] = isMissing(dblvalues[i]);
		} else if (values instanceof Character[]) {
			Character[] chrvalues = (Character[]) values;
			for (int i = 0; i < n; i++) missing[i] = isMissing(chrvalues[i]);
		} else {
			for (int i = 0; i < n; i++) missing[i] = (values[i] == null);
		}
		return missing;
	}
	...
	public static boolean areAnyMissing(Object[] values) {
		if (values instanceof String[]) {
			String[] strvalues = (String[]) values;
			for (String str : strvalues) if (isMissing(str)) return true;
			return false;
		} else if (values instanceof Double[]) {
			Double[] dblvalues = (Double[]) values;
			for (Double dbl : dblvalues) if (isMissing(dbl)) return true;
			return false;
		} else if (values instanceof Character[]) {
			Character[] chrvalues = (Character[]) values;
			for (Character chr : chrvalues) if (isMissing(chr)) return true;
			return false;
		} else {
			for (Object val : values) if (val == null) return true;
			return false;
		}
	}
	...
	public static int[] getNonMissingIndex(boolean[] missing) {
		int n = missing.length;
		int ntrue = 0;
		for (boolean miss : missing) if (!miss) ntrue++;
		int[] nonmiss = new int[ntrue];
		int count = 0;
		for (int i = 0; i < n; i++) if (!missing[i]) nonmiss[count++] = i;
		return nonmiss;
	}
    ...
    public static DoubleMatrix2D createFixedEffectArray(ArrayList<String[]> factorList, ArrayList<double[]> covariateList, ArrayList<Object> markerList, ArrayList<Object> markerIds, boolean[] missing) {
		...
		for (int c = 0; c < numberOfCovariates; c++) {
			...
			for (int i = 0; i < numberOfObs; i++) covArray.setQuick(i, 0, covar[nonmissingRows[i]]) ;
			fixedEffectsArray = DoubleFactory2D.dense.appendColumns(fixedEffectsArray, covArray);
		}

		//add markers;
		int numberOfMarkers;
		if (markerList == null) numberOfMarkers = 0;
		else numberOfMarkers = markerList.size();
		for (int m = 0; m < numberOfMarkers; m++) {
			...
			if (markerdata instanceof double[]) {
				...
				for (int r = 0; r < numberOfObs; r++) markermatrix.setQuick(r, 0, dblmarker[nonmissingRows[r]]);
				...
				for (int i = 0; i < numberOfObs; i++) markerLabels[i] = amarker[nonmissingRows[i]];
	...
	public static boolean isMissing(Object value) {
		...
		if (value instanceof Character) return value.equals('?');
		if (value instanceof String) {
			String val = (String) value;
			if (val.equals("?")) return true;
			if (val.equals("N")) return true;
			return false;
		}
		else return false;
	}
