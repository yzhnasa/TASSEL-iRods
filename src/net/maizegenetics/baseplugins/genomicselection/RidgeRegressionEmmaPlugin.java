...
public class RidgeRegressionEmmaPlugin extends AbstractPlugin {
	...
	public DataSet performFunction(DataSet input) {
    	...
    	if (datasets.size() < 1) {
    		...
    		myLogger.error(msg);
    		if (isInteractive()) JOptionPane.showMessageDialog(getParentFrame(), msg, "GS Error", JOptionPane.ERROR_MESSAGE);
    		return null;
    	}
    	...
    	for (Datum dataset : datasets) {
    		try {
    			...
    			e.printStackTrace();
    			if (isInteractive()) JOptionPane.showMessageDialog(getParentFrame(), msg.toString(), "GS Error", JOptionPane.ERROR_MESSAGE);
    		}
    	}
...
//    		theAdapter = new MarkerPhenotypeAdapter((MarkerPhenotype) dataset.getData());
    	...
    	for (int ph = 0; ph < numberOfPhenotypes; ph++) {
    		...
    		for (int m = 0; m < numberOfMarkers; m++) {
    			...
    			for (int i = 0; i < nObs; i++) genotype.set(i, m, ((Double) markerValue[i]).doubleValue());
    		...
    		for (int i = 0; i < nObs; i++) traitTable[i][0] = gebv[i];
